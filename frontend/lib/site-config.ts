...
export const SITE_CONFIG = {
  ...
  name: env.NEXT_PUBLIC_APP_NAME ?? "OpenWrt AI Copilot",
  tagline:
  ...
  company: env.NEXT_PUBLIC_COMPANY_NAME ?? "Necrodin",
  author: env.NEXT_PUBLIC_AUTHOR ?? "The OpenWrt AI Copilot contributors",
  license: env.NEXT_PUBLIC_LICENSE ?? "MIT",

  /** Versioning (build-time injected in the CI / Docker build). */
  version: env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0",
  frontendVersion: env.NEXT_PUBLIC_FRONTEND_VERSION ?? "0.0.0",
  gitCommit: env.NEXT_PUBLIC_GIT_COMMIT ?? null,
  buildDate: env.NEXT_PUBLIC_BUILD_DATE ?? null,
  ...
  socials: {
    github:
      ...
      "https://github.com/necrodin/openwrt-ai-copilot",
    openwrt: env.NEXT_PUBLIC_SOCIAL_OPENWRT ?? "https://openwrt.org",
  },
...
export function githubUrl(section: "repo" | "issues" | "releases" | "discussions" | "wiki"): string {
  switch (section) {
    case "repo":
